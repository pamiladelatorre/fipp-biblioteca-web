import { DataTable } from 'primereact/datatable';
import { Column } from 'primereact/column';
import { Button } from "react-bootstrap";
import { MovimentacaoEtapa } from "../../../enums/MovimentacaoEtapa";
import { MovimentacaoStatus } from "../../../enums/MovimentacaoStatus";

import { formatDateToBR } from '../../../utils/formatDate';

function MovimentacoesTable({ movimentacoes, onView }){
    // Exibe a etapa
    const etapaTemplate = (rowData) => MovimentacaoEtapa[rowData.etapa];

    // Exibe o status
    const statusTemplate = (rowData) => MovimentacaoStatus[rowData.status];

    // Exibe data inicio
    const dataInicioTemplate = (rowData) => formatDateToBR(rowData.dataInicio);

    // Exibe data prevista
    const dataPrevistaTemplate = (rowData) => formatDateToBR(rowData.dataPrevista);

    // Exibe data fim
    const dataFimTemplate = (rowData) => rowData.dataFim ? formatDateToBR(rowData.dataFim) : '-';

    // Exibe botões de editar
    const actionTemplate = (rowData) => {
        return (
            <div className='d-flex justify-content-center'>
                <Button variant="secondary" title="Ver detalhes" onClick={() => onView(rowData.id)} size="sm" className='rounded-pill'>
                    <i className='bi bi-eye'></i>
                </Button>
            </div>
        );
    };

    return(
        <DataTable 
            value={movimentacoes} 
            dataKey="id" 
            size='small'
            emptyMessage="Nenhum movimentação de exemplar encontrado." 
            paginator 
            rows={5} 
            rowsPerPageOptions={[5, 10, 25, 50]} 
            paginatorTemplate="RowsPerPageDropdown CurrentPageReport FirstPageLink PrevPageLink NextPageLink LastPageLink"
            currentPageReportTemplate="{first} - {last} de {totalRecords}"
            paginatorLeft={'Registro por página'}
            tableStyle={{ minWidth: '30rem' }}
        >
            <Column field="exemplar.acervo.titulo" header="Acervo" sortable></Column>
            <Column field="exemplar.tombo" header="Tombo"></Column>
            <Column field="usuario.nome" header="Usuário" sortable></Column>
            <Column field="etapa" header="Etapa" body={etapaTemplate}></Column>
            <Column field="status" header="Status" body={statusTemplate}></Column>
            <Column field="dataInicio" header="Data Início" body={dataInicioTemplate}></Column>
            <Column field="dataPrevista" header="Data Prevista" body={dataPrevistaTemplate}></Column>
            <Column field="dataFim" header="Data Fim" body={dataFimTemplate}></Column>
            <Column header="Ações" body={actionTemplate} style={{ width: '5rem' }}></Column>
        </DataTable>
    );
};

export default MovimentacoesTable;